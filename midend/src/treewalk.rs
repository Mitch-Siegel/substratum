...
use collect_ctx::UnpathedCollectCtx;
...
pub(crate) trait Collect<P>: ast::Ast
where
    P: symtab::Path,
{
    ...
    fn collect_symbols(&self, ctx: CollectCtx<P>) -> Result<CollectCtx<P>, CollectError> {
        let _ = trace::span_auto_debug!("Collect symbols for {Self::name()}");
...
pub(crate) fn walk(
    ...
    for module in &program {
        ...
        symtab = if module.name.value == crate_name {
            walk_items::walk_module::collect_from_crate_root(module, collect_ctx)
                .unwrap()
                .take()
        } else {
            ...
            walk_items::walk_module::collect_from_parent_path(
                module,
                collect_ctx.with_path(prefix_segments),
            )
            .unwrap()
            .take()
        }
    ...
    for module in program {
        ...
        if module.name.value == crate_name {
            ...
            let ((), ctx) = walk_items::walk_module::linearize_from_prefix_segments(
                module,

...
struct SymbolTable *linearizeProgram(struct AST *program)
{
	...
	Scope_addBasicBlock(programTable->globalScope, globalBlock);
	temps = TempList_New();
	...
	while (programRunner != NULL)
	{
		...
		case t_uint16:
		case t_uint32:
			walkVariableDeclaration(programRunner, globalBlock, programTable->globalScope, &globalTACIndex, &globalTempNum);
			break;
		...
		case t_fun:
			// walkFunction(programRunner, programTable->globalScope);
			break;
					...
					int *tempNum)
{
	...
	{
	case vt_uint32:
		assignedVariable = walkVariableDeclaration(lhs, block, scope, TACIndex, tempNum);

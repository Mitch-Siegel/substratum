...
void checkAccessedClassForDot(struct AST *tree, struct Scope *scope, struct Type *type)
{
    ...
    // make sure whatever we're applying the dot operator to is actually a class instance, or a single pointer
    if ((type->indirectionLevel > 1))
    {
        char *typeName = Type_GetName(type);
        if (tree->type == t_identifier)
        {
            ErrorWithAST(ERROR_CODE, tree, "Can't use dot operator on type with more than one level of indirection: %s %s\n", typeName, tree->value);
        }
        else
        {
            ErrorWithAST(ERROR_CODE, tree, "Can't use dot operator on type with more than one level of indirection: %s\n", typeName);
        }
    }
    else if (type->arraySize > 0)
    {
        char *typeName = Type_GetName(type);
        if (tree->type == t_identifier)
        {
            ErrorWithAST(ERROR_CODE, tree, "Can't use dot operator on array type: %s %s\n", typeName, tree->value);
        }
        else
        {
            ErrorWithAST(ERROR_CODE, tree, "Can't use dot operator on array type: %s\n", typeName);
        }
    }
}

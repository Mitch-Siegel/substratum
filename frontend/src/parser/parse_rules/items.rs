...
use crate::{
    ast,
    parser::parse_rules::{ItemParser, ParseError, Token, WorklistItem, parse_rules},
};
...
impl ItemParser<'_, '_> {
    fn parse_implementation_item(&mut self) -> Result<ast::ItemTree, ParseError> {
        Ok(ast::ItemTree::Implementation(self.parse_implementation()?))
    }

    fn parse_enum_definition_item(&mut self) -> Result<ast::ItemTree, ParseError> {
        Ok(ast::ItemTree::EnumDefinition(self.parse_enum_definition()?))
    }

    fn parse_struct_definition_item(&mut self) -> Result<ast::ItemTree, ParseError> {
        Ok(ast::ItemTree::StructDefinition(
    ...
    pub(crate) fn parse_item(
        &mut self,
        module_name: ast::IdentifierTree,
        module_path: &std::path::Path,
        crate_name: Option<&String>,
    ) -> Result<(ast::ItemTree, Option<BTreeSet<WorklistItem>>), ParseError> {
            ...
            Token::Mod => {
                ...
                match self.lookahead_token(2)? {
                    Token::LCurly => {
                        ...
                        Ok((ast::ItemTree::Module(Ok(module_tree)), Some(child_worklist)))
                    }
                    Token::Semicolon => {
                        ...
                        Ok((
                            ast::ItemTree::Module(Err((mod_loc, module_name.value))),
                            Some(child_worklist),
                        ))
                    }

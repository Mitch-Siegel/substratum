use crate::{
    ast,
    parser::parse_rules::{ExpressionParser, ParseError, Token},
};
...
impl ExpressionParser<'_, '_> {
    pub(crate) fn parse_expression(&mut self) -> Result<ast::Expression, ParseError> {
        let (_start_loc, _span) = self.start_parsing("expression")?;

        let mut expr = match self.peek_token()? {
            Token::SelfLower | Token::Identifier(_) => {
                ast::Expression::PathIn(self.parse_path_in_expression()?)
            }
            Token::If => ast::Expression::If(Box::new(self.parse_if_expression()?)),
            Token::Match => ast::Expression::Match(Box::new(self.parse_match_expression()?)),
            Token::While => ast::Expression::While(Box::new(self.parse_while_expression()?)),

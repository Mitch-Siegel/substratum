use crate::{
    ast,
    lexer::Token,
    parser::{ParseError, parse_rules::ItemParser},
};
...
impl ItemParser<'_, '_> {
    pub(crate) fn parse_function_declaration_or_definition(
        ...
        let decl_or_def = match self.parse_function_definition(prototype.clone()) {
            Ok(definition) => ast::ItemTree::FunctionDefinition(definition),
            Err(_) => ast::ItemTree::FunctionDeclaration(prototype),
        };
    ...
    pub(crate) fn parse_function_prototype(
        ...
        if allow_self_param && let Some(self_param) = self.try_parse_self_argument()? {

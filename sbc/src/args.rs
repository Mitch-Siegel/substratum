...
arg_struct! {
    Config {
        ...
        "-trace-level" => trace_level: trace::Level where default = trace::Level::ERROR, parse_fn = &mut |config, args| {
            config.trace_level = trace::Level::from_str(&args.next().expect("expected a value")).expect("malformed trace level string");
            true
        },
